...
sudo rm -f "$RESET_PLIST" /etc/sudoers.d/lid-awake "$HELPER_PATH" "$AGENT_PATH" "$CLI_PATH"
sudo rm -rf "$APP_PATH"
rm -f "$USER_AGENT_DIR/${POLICY_LABEL}.plist" "$USER_AGENT_DIR/${APP_LABEL}.plist"
rm -rf "$HOME/Library/Application Support/Lid Awake" "$HOME/Library/Logs/Lid Awake"
