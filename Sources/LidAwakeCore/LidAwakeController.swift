...
public struct LidAwakeController {
    ...
    public func loadSettings() -> LidAwakeSettings {
        guard let data = try? Data(contentsOf: Self.settingsFile),
              let value = try? JSONDecoder().decode(LidAwakeSettings.self, from: data) else {
            return LidAwakeSettings()
        }
        return value
    }
    ...
    public func requestEnabled(duration: Int? = nil) throws {
        ...
        if let duration {
            guard duration > 0 else { throw LidAwakeError.invalidDuration }
            settings.expiresAt = Date().addingTimeInterval(TimeInterval(min(duration, settings.maxDuration)))
        } else {
    ...
    public func requestTemporary(duration: Int) throws {
        try requestEnabled(duration: duration)
    }
    ...
    public func update(_ mutate: (inout LidAwakeSettings) throws -> Void) throws {
        ...
        try saveSettings(settings)
        try applyLaunchAtLogin(settings.launchAtLogin)
        _ = try reconcile()
    }
    ...
    public static func evaluate(settings: LidAwakeSettings, power: PowerInfo, thermal: ThermalLevel, now: Date) -> (LidAwakeState, String, LidAwakeSettings, Int?) {
        ...
        let reason = updated.expiresAt == nil
            ? L10n.text("Enabled until switched off", "Включено до ручного отключения")
            : L10n.text("Temporary mode is active", "Временный режим активен")
        return (.enabled, reason, updated, remaining)
    }

    @discardableResult public func reconcile(now: Date = Date()) throws -> LidAwakeStatus {
        let old = loadStatus()
        let initial = loadSettings()
        let power = readPowerInfo()
        let thermal = readThermalLevel()
        ...
        if old?.state != status.state || old?.reason != status.reason {
            appendEvent(status.reason)
            if status.settings.notifications { notifyTransition(from: old, to: status) }
        }
        return status
    }
    ...
    @discardableResult public func lockScreen() -> Bool {
        guard FileManager.default.isExecutableFile(atPath: Self.cgSessionPath) else {
            appendEvent(L10n.text("Could not lock screen: CGSession is unavailable", "Не удалось заблокировать экран: CGSession недоступен"))
            return false
        }
        let process = Process()
        process.executableURL = URL(fileURLWithPath: Self.cgSessionPath)
        ...
        if success { appendEvent(L10n.text("Screen locked after lid close", "Экран заблокирован после закрытия крышки")) }
        return success
    }
    ...
    public func applyLaunchAtLogin(_ enabled: Bool) throws {
        let uid = getuid()
        let target = "gui/\(uid)/\(Self.appLabel)"
    ...
    @discardableResult public func runHelper(_ command: String) throws -> String {
        ...
        guard process.terminationStatus == 0 else {
            throw LidAwakeError.commandFailed(stderr.trimmingCharacters(in: .whitespacesAndNewlines))
        }
    ...
    public func appendEvent(_ text: String) {
        ...
        if let handle = try? FileHandle(forWritingTo: Self.eventLogFile) {
            ...
            try? line.write(to: Self.eventLogFile, atomically: true, encoding: .utf8)
        }
    }

    private func notifyTransition(from old: LidAwakeStatus?, to status: LidAwakeStatus) {
        guard old != nil else { return }
        let message = status.reason.replacingOccurrences(of: "\"", with: "\\\"")
